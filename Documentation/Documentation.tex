...
\begin{document}
	...
	Il est nécessaire d'avoir au préalable installé \textbf{python3} et en option \textbf{xpdf} (sert à visualiser les pdf que l'on vient de générer mais peut fonctionner sans) sur son ordinateur pour exécuter le script ARO.
	
	Pour le bon fonctionnement il est nécessaire de garder le dossier \textit{.modele} qui contient toutes les informations nécessaires à la création d'un projet :
	...
	Ces deux packages permettent de compiler les fichiers latex généré, ceux fournis sont très dense car ce sont ceux que j'utilise personnellement mais ils peuvent être considérablement simplifié et ne contenir que ce qu'il faut pour créer les environnements décris dans le fichier \textit{environnement}.
