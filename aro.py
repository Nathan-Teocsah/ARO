...
class note:
    ...
    def montre(self,path_pdf_note):
        try :
            ...
            print("xpdf n'est pas installé ou pdf absent.")
...
\end{document}"""
    ...
    try :
        ...
        print("xpdf n'est pas installé ou abscence de pdf.")
...
def choix_description(path_tmp):
    ...
    if reponse=="e":
        compile_tmp(path_tmp,editeur_latex,"")
        latex = ""
        with open(path_tmp+"tmp.tex","r") as file :
            ligne = file.readline()
            while r"\end{document}" not in ligne :
...
def creer_relation_pour_note(Note1,Note2,type_Note,liste_note,path_pdf_lien,path_tmp,path_Note,filename_relation,Choix_rel):
    ...
    if k1 == len(liste_note):
        ...
        type1 = liste_note[k1].type
    if k2 == len(liste_note):
        type2 = type_Note
    else :
        type2 = liste_note[k2].type
...
def MENU(namedir_pdf_Note,namedir_pdf_Lien,namedir_tmp,namedir_document_maitre,namedir_document_chrono,filename_note,filename_relation,Protege,editeur_latex):
    ...
    def create_project():
        ...
        print(f"---> Projet '{name}' créé avec succès !\n-------------------------------------------\n")
        return name
